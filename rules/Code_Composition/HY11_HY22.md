...
> 本文件对应产出文档 [output/Code_Composition/HY11_HY22/HY11_HY22.md](../../output/Code_Composition/HY11_HY22/HY11_HY22.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
- **关键双侧目录/文件锚点**:
  - 脚本:`release/populate_prebuilt.sh`(`-v variant CRM variant (e.g. HY11, HY22, ...)`)
  - 清单文件:`release/crm/noship_common_HY11`/`noship_common_HY22`;`src/security/.../camera-release/crm/noship_common_HY11`/`HY22`/`HY11-HWASAN`;`src/display/vendor/qcom/proprietary/display-release/crm/noship_common_HY11`/`HY22`;`src/kernel-6.18/kernel_platform/qcom/proprietary/release/crm/noship_common_HY11`/`HY22`;`framework_release/crm/noship_common_HY11`/`HY22`
  - manifest属性:`.repo/manifests/default.xml`里`x-ship="hy11,hy22"`、`x-quic-distributable`
  - 业务差异证据:`release`仓库git历史(2954条commit,最早`f0a9f24`,2009年);命名沿革commit`7ce9ab4`(Add HY33 variant)、`429fe67`(hy33: Initial noship_common_HY33 for WebOS)、`3f2123c`(Rename HY33 to HY31 per target team request);`poky/meta-qti-distro/conf/distro/qti-distro-rb-debug.conf`(`DISTRO_NAME`+`ros2`/`vslam`/`librealsense2` DISTRO_FEATURES)
  - QLI2.0替代分发机制recipe:`qwes_1.1.bb`、`diag-router_1.0.2.bb`、`firmware-qcom-boot-qcs9100.inc`(均指向`softwarecenter.qualcomm.com`)
- **已验证的检索方式**:
  - 全树扫描(含`.bb`/`.bbappend`/`.conf`/`.md`/`.txt`/`.inc`/`.yml`/`.yaml`,排除build产物)检索"HY11"/"HY22"字符串
  - 读取`release/populate_prebuilt.sh`里`-v variant`用法说明及`if [ "$variant" == "HY11" ] ...`分支逐字核对功能定位
  - `diff crm/noship_common_HY11 crm/noship_common_HY22`比较两变体清单差异,坐实HY22独有的调制解调器相关路径(`mdm-ss-mgr/{diag-reboot-app,pdc-daemon}`、`qmi`、`mbim`、`gps`等)
  - `release`仓库`git log --all --grep`跨2009年至今全部历史搜索HY编号命名沿革
  - `grep -rn "softwarecenter.qualcomm.com" meta-qcom --include=*.bb --include=*.bbappend --include=*.inc`定位QLI2.0替代分发路径
- **已知易错点/纠错记录**:(暂无README纠错表条目式的"结论被推翻"记录)本文档对"HY"缩写全称的排查是渐进收窄式的,提示单纯全树关键字检索("HY11"/"HY22"本身)不足以回答"HY"字面全称问题——需要扩展到`release`仓库完整git commit历史(含2009年WebOS/HY31/HY33相关commit)才能坐实HY编号是"target team按需分配、可重命名的序号"而非固定语义缩写;字面全称本身仍未找到,已记录为"待确认"而非强行下结论,复核时勿把"关键字搜索为空"误判为"该编号无历史记录"。
