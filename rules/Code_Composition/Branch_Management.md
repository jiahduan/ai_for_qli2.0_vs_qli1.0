# Branch_Management —— 规则

> 本文件对应产出文档 [output/Code_Composition/Branch_Management/Branch_Management.md](../../output/Code_Composition/Branch_Management/Branch_Management.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
## Branch_Management专属取证要点

- **关键双侧目录/文件锚点**:
  - downstream(maili):`.repo/manifests/default.xml`(manifest revision字段);子仓库HEAD抽样`qc/display-kernel.lnx/cd`、`qc/camx.lnx/cd`、`qc/wlan-cmn.driver.lnx/cd`、`qc/audio-ahal-handset.lnx/cd`、`qc/gfx-kernel.lnx/cd`、`src/security/securemsm`、`poky`、`poky/meta-qti-bsp`
  - QLI2.0:`meta-qcom`(`wrynose`分支)、`meta-qcom-distro`、`meta-qcom-robotics-sdk`、`meta-audioreach`、`meta-updater`、`meta-security`、`meta-selinux`、`meta-virtualization`、`oe-core`、`bitbake`、`meta-openembedded`、`meta-ros`、`meta-lts-mixins`
  - `base.lock.yml`(各产品顶层repo的中心化commit锁定文件)
  - `patched-<原始commit sha>`(kas对被`patches:`声明打了本地补丁的仓库自动生成的临时本地分支,无remote)
  - `backport/*-to-wrynose`临时分支(如`remotes/origin/backport/2951-to-wrynose`)
- **已验证的检索方式**:
  - 逐仓库读取`git log`/`git describe`的HEAD commit hash与message,核对是否携带`LA.VENDOR.*`/`sec-userspace.lnx.*`等版本tag字符串
  - `git log --oneline -- ci/base.lock.yml`查看lock文件更新记录是否走正常PR+`[Backport wrynose]`模式
  - 匿名`curl https://api.github.com/repos/qualcomm-linux/<repo>/branches/<branch>`读取`protected`字段(公开仓库无需认证)
  - `curl https://api.github.com/repos/{owner}/{repo}/rulesets/{id}`读取具体ruleset规则详情(如`meta-qcom`ruleset id 4151135、`meta-qcom-distro` id 2421982、`meta-audioreach` id 10152596),比传统`.../branches/{branch}/protection`端点(匿名会401/403)更完整
- **已知易错点/纠错记录**:(暂无纠错记录;本文档未出现"初步判断→核实后结论"的对照,`patched-<sha>`分支机制本身是Patch_Management.md纠错案例中提到的关联产物,但不是本主题自身的纠错记录)
