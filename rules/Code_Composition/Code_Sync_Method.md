...
> 本文件对应产出文档 [output/Code_Composition/Code_Sync_Method/Code_Sync_Method.md](../../output/Code_Composition/Code_Sync_Method/Code_Sync_Method.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
- **已验证的检索方式**:
  - `grep -rl "manifest.xml\|<project \|repo init"`(排除`build/`产物目录)全树检索repo manifest残留,核实"是否还有repo痕迹"
  - 直接读取本机`kas`发行包源码(`site-packages/kas/libcmds.py`)反查凭证注入逻辑(SSH_PRIVATE_KEY自动起`ssh-agent`、HTTPS走`GIT_CREDENTIAL_HELPER`/`.netrc`、GitLab CI下自动追加`gitlab-ci-token`),这是"读取工具源码具体字段"式取证,不是QLI2.0代码库本身的证据
  - 全树检索`netrc|GIT_ASKPASS|ssh-agent|known_hosts|credential.helper|KAS_REPO_REF_DIR`确认QLI2.0快照内是否已配置私有仓库接入
- **已知易错点/纠错记录**:(暂无纠错记录;文档"待确认"部分是尚未查清的开放问题——QLI2.0具体启用哪种kas鉴权机制、私有fork托管在哪——不是被推翻的旧结论)
