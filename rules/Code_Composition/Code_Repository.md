...
> 本文件对应产出文档 [output/Code_Composition/Code_Repository/Code_Repository.md](../../output/Code_Composition/Code_Repository/Code_Repository.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
## Code_Repository专属取证要点

- **关键双侧目录/文件锚点**:
  - downstream(maili):`.repo/projects/`、`.repo/project-objects`(89个project-object目录);符号链接样例`poky/.git`、`src/security/securemsm/.git`、`qc/display-kernel.lnx/cd/.git`
  - QLI2.0:`meta-qcom/.git`、`meta-audioreach/.git`、`meta-updater/.git`、`oe-core/.git`、`bitbake/.git`(均为真实目录)
  - `meta-qcom/ci/base.lock.yml`与`meta-qcom-robotics-sdk/ci/base.lock.yml`(内容完全一致,锁定`oe-core`/`meta-lts-mixins`/`bitbake`/`meta-arm`/`meta-qcom-distro`/`meta-openembedded`/`meta-virtualization`/`meta-audioreach`/`meta-selinux`/`meta-updater`/`meta-security`/`meta-dpdk`共12个repo)
  - `meta-qcom-robotics-sdk/ci/qcom-robotics-distro.yml`(产品顶层repo自己单独锁commit的位置,不在`base.lock.yml`内)
- **已验证的检索方式**:
  - `ls -la poky/.git src/security/securemsm/.git qc/display-kernel.lnx/cd/.git`查符号链接实际指向
  - `stat -c "%N type=%F" meta-qcom/.git meta-audioreach/.git meta-updater/.git oe-core/.git bitbake/.git`确认是否为真实git目录而非软链
  - 读取并对比`meta-qcom/ci/base.lock.yml`与`meta-qcom-robotics-sdk/ci/base.lock.yml`的repo/commit清单是否一致
  - `du -sh */.git`(文档中提出但标注为"未做定量对比"的存储占用核实思路,后续复核时可直接用)
- **已知易错点/纠错记录**:(暂无纠错记录)
