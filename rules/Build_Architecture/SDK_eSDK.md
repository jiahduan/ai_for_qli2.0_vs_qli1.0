...
> 本文件对应产出文档 [output/Build_Architecture/SDK_eSDK/SDK_eSDK.md](../../output/Build_Architecture/SDK_eSDK/SDK_eSDK.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
- **已验证的检索方式**:
  - `git log`逐个产品镜像recipe(`qti-multimedia-image.bb`、`qti-xreality-image.bb`、`qti-xreality2-base-image.bb`、`qti-robotics-image.bb`)取最后一次真实改动时间,剔除本地`[DNM]`临时commit,用于判断该SKU代码维护活跃度
  - `git log`分别核对`qcom-base.inc`里`DISTRO_VERSION`赋值历史与`psdk-image.bbclass`里`SDK_VERSION`赋值历史,对比两者是否在同一时间段各自独立变化,判断是否脱钩发布节奏
  - grep核实meta-qcom/meta-qcom-distro/meta-security/meta-updater/meta-virtualization全层是否存在multimedia/networking/container-orchestration专属SDK打包的TODO/WIP痕迹(结果为无,唯一命中`do_populate_sdk`的`meta-virtualization`条目与Qualcomm产品线规划无关)
  - 读取`qti-xreality2-image.bb`的`require qti-xreality2-base-image.bb`引用链,确认是否同样`inherit populate_sdk_qti`(用于判断某产品线是否真的在用eSDK机制,而非仅凭recipe文件名猜测)
- **已知易错点/纠错记录**:(暂无纠错记录)
