...
- **已验证的检索方式**
  ...
  - 全局检索`UKI_SB_KEY`/`UKI_SB_CERT`/`sbsign`,范围限定`meta-qcom*`/`build/conf`——零命中,用于判断UKI是否走secure boot签名
  - 全局检索`fs-verity`/`fsverity`/`veritysetup`/`dm-verity`,范围限定`meta-qcom*`——零命中,用于判断根文件系统完整性校验替代方案是否存在
  - 读取`pebble.conf`的`CONSOLE_PARAM:qti-distro-perf`字段、`esp-qcom-image.bb`的`UKI_CMDLINE`字段,直接对比两侧cmdline拼接方式
  - 读取`esp-qcom-image.bb`里`UKI_CMDLINE`是否含`rw`/`ro`,以及`DISTRO_FEATURES`/`EXTRA_IMAGE_FEATURES`是否含`read-only-rootfs`,用于判断是否存在只读根替代方案
- **已知易错点/纠错记录**
  - 本文档"影响与风险"一节明确指出:cmdline构建期固化"不能简单等同于更安全",初步容易读成"固化=更安全",但核实后结论是"固化只带来可预测性/可审计性,尚未带来防篡改能力"(因UKI签名钩子零命中)
  - README《曾纠正过的结论》表收录条目"QLI2.0 UKI是否签名(Boot_Flow/Bootargs)":初步表述为"构建期静态烘焙进UKI,签名后不可变",暗示已走secure boot签名流程;核实后更正为`UKI_SB_KEY`/`UKI_SB_CERT`/`sbsign`全局检索零命中,当前默认构建的UKI实际未签名
