...
- **已验证的检索方式**
  ...
  - 读取`meta-qcom/classes-recipe/image_types_qcom.bbclass`第127-136行,判断默认闭源固件路径下u-boot编译产物是否参与打包(结论:不参与,只在`open-fw`配置下才把`u-boot.mbn`改名为`uefi.elf`)
  - 读取`qcom-u-boot-common.inc`第13行的`UBOOT_CONFIG[iq-9075-evk]`字段确定具体defconfig
  - 读取`meta-qcom-distro/conf/distro/include/qcom-base.inc`第56行的`INITRAMFS_IMAGE`变量及其注释,确认initrd是否仍被使用(而非直接认为"vendor_boot消失=initrd消失")
- **已知易错点/纠错记录**
  - 文档内明确记录:"初步判断'签名后不可变',核实后更正为'当前未签名'"——即构建期静态烘焙cmdline进UKI,容易误读为"签名后不可变",但`UKI_SB_KEY`/`UKI_SB_CERT`/`sbsign`全局检索零命中,实际未做secure boot签名
  - 同一条目也收录在README《曾纠正过的结论》表:"QLI2.0 UKI是否签名(Boot_Flow/Bootargs)"
  - 易错点提醒:initramfs/ramdisk"看似消失"实为Android式vendor_boot分区概念消失,initrd本体仍以`initramfs-rootfs-image`形式内嵌进UKI,不应直接断言"无initrd启动"
