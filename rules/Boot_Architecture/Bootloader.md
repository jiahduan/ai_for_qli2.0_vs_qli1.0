...
> 本文件对应产出文档 [output/Boot_Architecture/Bootloader/Bootloader.md](../../output/Boot_Architecture/Bootloader/Bootloader.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
- **已验证的检索方式**
  ...
  - 读取`qcom-u-boot-common.inc`第13行的`UBOOT_CONFIG[<machine>]`字段确定板级defconfig
  - 读取`qtestsign_git.bb`的`SRC_URI`字段,判断签名工具来源是QTI私有还是社区(msm8916-mainline)维护
  - 读取`qsigning.bbclass`里实际调用sectools时的`--signing-mode`参数值(结论:硬编码`TEST`,非`PLUGIN`)
