...
> 本文件对应产出文档 [output/Platform_Features/OTA_Mechanism.md](../../output/Platform_Features/OTA_Mechanism.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
- **关键双侧目录/文件锚点**:
  - QLI1.0核心OTA:`src/OTA/`(AOSP`bootable/recovery`+`build/tools/releasetools`+`system/update_engine`移植:`recovery/{applypatch,edify,updater,minzip,minui,bootloader_message,update_verifier,otafault}`)、`external/bsdiff/`、`external/libdivsufsort/`
  - QLI1.0 A/B切换:`src/bootctrl/abctl/`(`libabctl.cpp`/`abctl.cpp`,命令`--set_active`/`--boot_slot`/`--set_success`/`--set_unbootable`/`--set_priority`)、`ab-updater.service`、`ab-symlink.service`
  - QLI1.0升级包:`build/tools/full_ota.sh`、`build/tools/incremental_ota.sh`、`build/tools/releasetools/ota_from_target_files.py`
  - QLI1.0:`src/OTA/recovery/update_engine/update_engine.cpp`(`#ifdef TARGET_SUPPORTS_AB`/`#include <libabctl.h>`,`ABC_OTA_STATUS_COOKIE_FILE="/cache/recovery/abc_ota_status"`);`qc/le-ota.lnx/cd/cd.xml`(`oem-recovery`/`recovery`/`recovery-ab`三个包)
  - dm-verity/AVB(QLI1.0):`poky/meta-qti-bsp/classes/`下8个bbclass——`dm-verity-initramfs.bbclass`、`dm-verity-initramfs-v2/v3.bbclass`、`dm-verity-bootloader.bbclass`、`dm-verity-cpio-cmdline.bbclass`、`dm-verity-none.bbclass`、`avb-verity-initramfs.bbclass`;相关补丁`fstab-generator-Honor-verity-enabled-cmdline.patch`
  - QLI2.0核心:`meta-updater/README.adoc`;`recipes-sota/ostree`、`recipes-sota/aktualizr/`(`aktualizr_git.bb`、`aktualizr-device-prov.bb`、`aktualizr-device-prov-hsm.bb`、`aktualizr-shared-prov*.bb`、`aktualizr-uboot-env-rollback.bb`及其安装的`30-rollback.toml`)
  - QLI2.0 opt-in配置:`qcom-distro-sota.conf`(`require conf/distro/sota.conf.inc`;`INITRAMFS_IMAGE="initramfs-ostree-image"`;`DISTRO_NAME:append=" (OTA-enabled)"`;`BUILD_OSTREE_REPO_TARBALL="1"`);对应CI:`meta-qcom/ci/qcom-distro-sota.yml`
  - dm-verity/AVB等价能力(QLI2.0,存在但未启用):`meta-security/classes/dm-verity-img.bbclass`、`meta-security/meta-integrity/classes/ima-evm-rootfs.bbclass`;`qcom-base.inc`(`DISTRO_FEATURES:append`不含`ima`)
  - 交叉引用文档:output/Code_Composition/Patch_Management.md(3个systemd补丁去向)、output/Boot_Architecture/Bootargs.md(cmdline无verity参数)、output/Boot_Architecture/Partition_Layout.md(Android安全HAL分区消失、单一rootfs分区)
