...
> 本文件对应产出文档 [output/System_Architecture/Distro_Version.md](../../output/System_Architecture/Distro_Version.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
## Distro_Version专属取证要点

- **关键双侧目录/文件锚点**
  - QLI1.0:`meta-qti-distro`(30个`qti-distro-{base,fullstack,camerastack,xr,vnm,rb,host}-*.conf`变体)、`qti-distro-base.inc`(DISTRO_FEATURES专有追加)、`qti-distro-camerastack-debug.conf`(`PREFERRED_PROVIDER_virtual/refpolicy="refpolicy-mls-robotics"`)
  - QLI2.0:`meta-qcom-distro`(`qcom-distro.conf`/`qcom-distro-selinux.conf`/`qcom-distro-sota.conf`/`qcom-distro-catchall.conf`)、`qcom-base.inc`(DISTRO_FEATURES:append)、`meta-qcom-robotics-sdk`(`qcom-robotics-distro-{catchall,selinux,sota}.conf`)、`meta-qcom-distro/ci/`(35个yml,如`debug.yml`/`performance.yml`)
- **已验证的检索方式**
  - 逐项对比`qti-distro-base.inc`与`qcom-base.inc`的DISTRO_FEATURES字段列表,确认两侧专有追加项完全不重叠
  - 遍历`meta-qcom-distro/ci/`全部35个yml文件及`meta-qcom/ci/`,检索是否存在`camerastack`/`xr`/`vnm`/`host`对应kas片段,确认零命中
  - 读取`meta-qcom-distro`的git创建/首次提交日期及commit总量(自2023-07-18创建、2024-01-25首次实质提交至今582个commit),判断演进活跃度而非"尚未展开的精简阶段"
- **已知易错点/纠错记录**:(暂无纠错记录)
