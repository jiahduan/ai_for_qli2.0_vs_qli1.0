...
> 本文件对应产出文档 [output/System_Architecture/Yocto.md](../../output/System_Architecture/Yocto.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
- **已验证的检索方式**
  ...
  - `grep -rlE 'virtual/\$\{(TARGET|HOST|SDK)_PREFIX\}' meta-qti-*`(实测0命中)
  - `grep -rl "PKG_CONFIG_PATH" meta-qti-*`(实测3个命中)
  - 全层(`meta-qcom`/`meta-qcom-distro`/`meta-audioreach`/`meta-security`/`meta-updater`)grep`cve-check`/`create-spdx`/`sbom-cve-check`/`INHERIT.*cve-check`,确认零命中
  - 读取`oe-core/meta/classes-recipe/uboot-config.bbclass`解析逻辑(仅`len(items) > 1`时触发legacy警告),对比`qcom-u-boot-common.inc`全部7机型`UBOOT_CONFIG`写法(均为单段式,无legacy风险)
  - 因本地网络受限无法直连官网,先将`docs.yoctoproject.org`的migration/release notes页面存档到`reference/System_Architecture/Yocto/`后再逐条核实breaking change原文
- **已知易错点/纠错记录**:(暂无纠错记录;文档内多处是"已核实吻合"式确认,未见"初步判断→核实后结论"式反转框架)
