...
> 本文件对应产出文档 [output/System_Architecture/RT.md](../../output/System_Architecture/RT.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
- **关键双侧目录/文件锚点**
  - QLI1.0:`linux-yocto-rt_6.6.bb`(上游样板,未接线)、`arch/arm64/configs/{defconfig,gki_defconfig,*.fragment}`、`poky/meta-qti-distro`全目录、`kernel_platform/soc-repo`(remote`quic/qcom-6.18`)与`kernel_platform/common`(remote`quic/keystone/android17-6.18-keystone-qcom-release`)
  - QLI2.0:`linux-qcom-rt_6.18.bb`(`require linux-qcom_6.18.bb`)、`linux-qcom-next-rt_git.bb`、`arch/arm64/configs/rt.config`(`CONFIG_EXPERT=y`/`CONFIG_PREEMPT_RT=y`)、`meta-qcom/conf/machine/include/qcom-common.inc`(`QCOM_RT_CPU`→isolcpus、`QCOM_IRQAFF`→irqaffinity、`QCOM_RCU_NOCBS`、`QCOM_RCU_EXPEDITED`、`QCOM_CPUIDLE_OFF`)、`ci/linux-qcom-rt-6.18.yml`/`linux-qcom-next-rt.yml`、`qcom-armv8a.conf`(默认仍`linux-yocto`)、`qcom-minimal-image.bb`(RT provider下自动装`rt-tests`)
- **已验证的检索方式**
  - 全目录grep`CONFIG_PREEMPT_RT`/`rt-tests`,遍历`arch/arm64/configs`全部defconfig/fragment/build.config及`poky/meta-qti-distro`全include链,确认零命中
  ...
  - 逐一核对`meta-qcom`全部19个machine conf(及`require`链)是否设置`QCOM_RT_CPU`,统计已赋值机型清单(≥9个,均为IQ工控评估板/机器人核心套件)
- **已知易错点/纠错记录**:(暂无纠错记录)
