# Security_Architecture —— 规则

> 本文件对应产出文档 [output/System_Architecture/Security_Architecture/Security_Architecture.md](../../output/System_Architecture/Security_Architecture/Security_Architecture.md),与全部33份主题规则文件按本次目录重构选择的方式各自完整独立(7条强制规则全文一致,不做共享继承,变更时需同步维护;背景见[Scope_Section_Design.md](../../Scope_Section_Design.md))。全局工作流/与README关系见根目录[Methodology.md](../../Methodology.md)。
...
- **已验证的检索方式**
  ...
  - 改用`git log`排查`meta-qcom/dynamic-layers/selinux`目录提交历史,逐条读commit message定位真实策略点(nhx.sh、tee_supplicant_qtee、pd-mapper)
  - `git log HEAD..origin/wrynose -- dynamic-layers/selinux`核实本地checkout落后主线的commit数(317个)及其中SELinux相关路径差异(仅3条)
  - grep`apparmor`/`firejail`/`seccomp`/IMA相关recipe,核实是否被任何qcom镜像IMAGE_INSTALL引用,区分"层里有、没人用"的死代码
  - grep`cd.xml`/`compliance`全目录(零命中),交叉核实`SPDX_INCLUDE_KERNEL_CONFIG ?= "1"`及实际构建产物`.spdx.json`确认替代合规机制
- **已知易错点/纠错记录**:有(与README一致)——初步判断"QLI2.0侧SELinux策略完全空白"(按`.te`/`.fc`/`.if`扩展名全量检索仅命中6个mariadb通用文件,与Qualcomm定制无关);核实后结论"有真实但规模远小的雏形"(QLI2.0定制策略以patch形式内嵌进refpolicy-targeted源码树,按扩展名搜索必然漏判,改用git log排查后确认3个真实策略点)。已同步进README《曾纠正过的结论》表。
