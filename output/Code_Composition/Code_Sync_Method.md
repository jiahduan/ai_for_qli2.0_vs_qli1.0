...
- **明确排除**:
  - kas`patches:`跨仓补丁机制本身(字段语义与消费方patches目录关系) ——见[Patch_Management](Patch_Management.md)
  - kas`patches:`产生的`patched-<hash>`分支命名与状态 ——见[Branch_Management](Branch_Management.md)
  - Gerrit评审/GitHub PR code review流程对照 ——见[Code_Submission](../Platform_Features/Code_Submission.md)
  - `base.lock.yml`锁定的具体repo清单及与各产品顶层yml的两层锁定结构细节 ——见[Code_Repository](Code_Repository.md)
...
| 版本追溯方式 | Gerrit评审+manifest revision(详见output/Platform_Features/Code_Submission.md) | 上游开源、轻量,不再需要访问Qualcomm内部gerrit/repo服务器 |
...
- 失去repo的`local_manifests`灵活覆盖能力和Gerrit code-review集成(`review-android.quicinc.com`),需确认QLI2.0侧的code review流程(GitHub PR?)是否已配套(详见output/Platform_Features/Code_Submission.md)。
