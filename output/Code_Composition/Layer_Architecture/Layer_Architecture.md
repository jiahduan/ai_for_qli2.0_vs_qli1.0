...
- **覆盖**:
  - 激活层数量统计:重新逐行核实downstream(maili)`build-qti-distro-camerastack-debug/conf/bblayers.conf`的`BBLAYERS`(实测59条路径)与QLI2.0`build/conf/bblayers.conf`的`BBLAYERS`(实测21条`${TOPDIR}/../`条目),两次重新统计与现有正文结论完全一致
  - downstream(maili)`poky/meta-qti-*`系列层全量普查(重新`find poky -maxdepth 1 -iname "meta-qti-*"`实测49个子目录,含prop/internal/core/kernel等后缀细分;文档正文"约45个"为口径宽松估计,已记入本次报告的偏差说明)与QLI2.0全部21个激活层名单、"旧layer→新layer映射表"
...
- **明确排除**:
  - WLAN/BT驱动栈、平台守护进程与固件深度对比 ——见[WiFi_BT](../../System_Architecture/WiFi_BT/WiFi_BT.md)
  - Toolchain/`meta-clang`组织形式变化与内核构建clang强制使用问题的深度分析 ——见[Toolchain](../../Build_Architecture/Toolchain/Toolchain.md)
  - OTA机制(`src/OTA`→`meta-updater`)深度对比 ——见[OTA_Mechanism](../../Platform_Features/OTA_Mechanism/OTA_Mechanism.md)
  - `meta-lts-mixins`与内核版本管理关系的辨析 ——见[Kernel_Code_Architecture](../../System_Architecture/Kernel_Code_Architecture/Kernel_Code_Architecture.md)
  - Display合成服务层/DRM-KMS/Wayland/Mesa深度对比 ——见[Display](../../System_Architecture/Display/Display.md)
  - GPU用户态3D驱动开源状态深度对比 ——见[Graphics](../../System_Architecture/Graphics/Graphics.md)
  - Camera CamX/ChiCDK/QMMF/camera-service深度对比 ——见[Camera](../../System_Architecture/Camera/Camera.md)
  - Audio框架(`meta-iot-audio`→`meta-audioreach`)深度对比 ——见[Audio](../../System_Architecture/Audio/Audio.md)
  - SELinux/`meta-qti-sepolicy`/`meta-selinux`策略体系深度对比 ——见[Security_Architecture](../../System_Architecture/Security_Architecture/Security_Architecture.md)
...
| downstream(maili)旧layer | QLI2.0新layer/归属 | 说明 |
|---|---|---|
...
| meta-qti-wlan(-prop) | **meta-qcom**(recipes-connectivity/sigma-dut) | 未见独立wlan专有层,ath系开源驱动路径已在output/System_Architecture/WiFi_BT/WiFi_BT.md完整核实 |
...
| meta-clang | **组织形式变化,已合并进oe-core主干** | 详见output/Build_Architecture/Toolchain/Toolchain.md,非能力删除 |
...
| meta-qti-security-internal-common | 无法评估(downstream(maili)侧本就是空层) | 证据不足 |
...
- 层数从59条压缩到21条,降幅约64%,但这个降幅是三种不同性质的变化叠加而成,不能直接读成"功能砍掉了六成":大部分`meta-qti-*`是被合并进`meta-qcom`/`meta-qcom-distro`(硬件使能层整合,能力延续、只是物理位置变了),部分整层拆成独立开源产品(`meta-audioreach`、`meta-updater`),同时还新增了downstream(maili)完全没有的机器人/ROS产品线专属层(`meta-qcom-robotics-sdk`、`meta-ros`)。三者相加才导致净层数大幅下降。
...
- 工具链从`meta-clang`移除后如果编译器切换到纯GCC,可能影响之前依赖clang特定优化/sanitizer(如HWASAN,注意downstream(maili)中出现过`noship_common_HY11-HWASAN`文件名)的调试能力(详见output/Build_Architecture/Toolchain/Toolchain.md,已纠正为组织形式变化而非能力删除)。
