...
| — | **meta-updater**(全新,aktualizr/OSTree OTA) | 功能上取代QLI1.0`src/OTA`(Android式bsdiff+recovery A/B升级),详见output/Platform_Features/OTA_Mechanism/OTA_Mechanism.md |
...
- 工具链从`meta-clang`移除后如果编译器切换到纯GCC,可能影响之前依赖clang特定优化/sanitizer(如HWASAN,注意QLI1.0中出现过`noship_common_HY11-HWASAN`文件名)的调试能力(详见output/Build_Architecture/Toolchain/Toolchain.md,已纠正为组织形式变化而非能力删除)。
