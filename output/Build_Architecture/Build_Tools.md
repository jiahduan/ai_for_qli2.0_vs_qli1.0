...
- **明确排除**:
  - kas代码同步/版本锁定机制的完整对比(repo vs kas元数据规模、工作树落地方式、锁定粒度、容器化) ——见[Code_Sync_Method](../Code_Composition/Code_Sync_Method.md)
  - kas`patches:`跨仓补丁声明机制 ——见[Patch_Management](../Code_Composition/Patch_Management.md)
  - host环境最低要求(Python/磁盘/内存/shell强制检查) ——见[Build_Environment](Build_Environment.md)
