...
- **明确排除**:
  - cmdline具体参数内容/逐机型取值(perf构建字符串本身、verity/AVB相关参数检索、`KERNEL_CMDLINE_EXTRA`逐机型追加项、console参数规范化) ——见[Bootargs](Bootargs.md)
  - Bootloader自身的来源可追溯性、签名机制(sectoolv2/qtestsign)、板级defconfig矩阵、开放固件变体细节 ——见[Bootloader](Bootloader.md)
  - Android安全HAL分区消失清单、分区总数对比 ——见[Partition_Layout](Partition_Layout.md)
  - dm-verity/AVB完整性校验替代方案的产品级结论 ——见[Bootargs](Bootargs.md)
  - OTA/Recovery触发路径与升级失败回滚SOP ——见[OTA_Mechanism](../Platform_Features/OTA_Mechanism.md)
...
- Recovery/OTA触发路径完全不同,涉及售后维修、字段升级失败恢复流程都要重新制定SOP(详见output/Platform_Features/OTA_Mechanism.md)。
