...
- **明确排除**:
  - UKI secure boot签名机制本身的取证与权威结论(`UKI_SB_KEY`/`UKI_SB_CERT`/`sbsign`零命中的判定,及该结论在README的纠错记录归属) ——见[Boot_Flow](../Boot_Flow/Boot_Flow.md)
  - 启动链各阶段角色、Bootloader身份、内核镜像格式、initramfs机制、root挂载执行主体 ——见[Boot_Flow](../Boot_Flow/Boot_Flow.md)
  - systemd层fstab-generator补丁的具体改动内容与吸收判定过程 ——见[systemd_](../systemd_/systemd_.md)
  - Android安全HAL分区消失的具体清单与分区总数对比 ——见[Partition_Layout](../Partition_Layout/Partition_Layout.md)
  - OTA/Recovery完整性校验替代方案的产品级决策 ——见[OTA_Mechanism](../../Platform_Features/OTA_Mechanism/OTA_Mechanism.md)
