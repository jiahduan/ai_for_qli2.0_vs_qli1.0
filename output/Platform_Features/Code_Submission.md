...
- **明确排除**:
  - 版本管理拓扑本身(repo manifest结构 vs 各层独立git仓库)的深度对比 ——见[Code_Repository](../Code_Composition/Code_Repository.md)
  - 代码同步/拉取机制本身(`repo sync`命令层面 vs `kas checkout`/`git clone`)的深度对比 ——见[Code_Sync_Method](../Code_Composition/Code_Sync_Method.md)
  - kas CI矩阵机制本身(`ci/*.yml`结构、`kas build`触发方式) ——见[Build_Tools](../Build_Architecture/Build_Tools.md)
  - GitHub分支保护/ruleset的存在性与`non_fast_forward`(禁止force push)规则本身 ——见[Branch_Management](../Code_Composition/Branch_Management.md)
