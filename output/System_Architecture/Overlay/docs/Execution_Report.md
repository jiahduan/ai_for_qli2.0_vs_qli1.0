...
**落到结论**:对比总览表(rootfs)"实现方式""是否默认启用"两行,"影响与风险"节"当前默认构建形态下既没有QLI1.0式overlay-mounter,也没有开启OSTree原子只读根"的判断。
...
**落到结论**:"关键差异"节"QLI1.0的techpack overlay已确认是真实交付内容而非死代码"的判断。
