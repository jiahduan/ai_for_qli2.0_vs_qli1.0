#!/usr/bin/env bash
# 薄包装脚本：对应产出文档 output/Boot_Architecture/Boot_Flow/Boot_Flow.md 的范围校验入口。
#
...
#
# 用法: scripts/Boot_Architecture/Boot_Flow_check.sh

set -uo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
TOPIC_FILE="Boot_Flow.md"

out="$("$REPO_ROOT/scripts/check_scope_links.sh" 2>&1 || true)"
filtered="$(printf '%s\n' "$out" | grep -F "$TOPIC_FILE" || true)"

if [ -z "$filtered" ]; then
  echo "[output/Boot_Architecture/Boot_Flow/Boot_Flow.md] 未在全局校验输出中被提及(可能已合规，也可能是本主题目前还没有《对比范围》节——不能替代人工确认)"
  exit 0
fi
