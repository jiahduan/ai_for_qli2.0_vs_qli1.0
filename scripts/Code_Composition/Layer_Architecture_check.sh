#!/usr/bin/env bash
# 薄包装脚本：对应产出文档 output/Code_Composition/Layer_Architecture.md 的范围校验入口。
#
...
if [ -z "$filtered" ]; then
  echo "[output/Code_Composition/Layer_Architecture.md] 未在全局校验输出中被提及(可能已合规，也可能是本主题目前还没有《对比范围》节——不能替代人工确认)"
  exit 0
fi
